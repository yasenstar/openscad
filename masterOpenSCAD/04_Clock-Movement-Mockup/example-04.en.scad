...
/*
    clock_movement library
    
    usage:
    
    clock_movement( time = [12, 0, 0], hand_style = "modern");
        
    - parameter time is a vector with [hours, minutes, seconds]
    - parameter hand_style can have the values "modern", "classic", "deco"

    dimensions of the clock_movement and the hand_style can be accessed via
    
    - movement_dim( name );
    - hands_dim( name, style );
    
    An overview of the available data is provided by the following arrays
...
module clock_movement( time = [12, 0, 0], hand_style = "modern") {
    ...
    difference(){
        ...
        // flattening and notch
        color("Gray")
        for ( i = [0:1])
        mirror( [i, 0, 0] )
        union(){
            translate([
                alignment_dm / 2 - alignment_flat,
                -(alignment_dm + 2) / 2,
                -1
            ])
            cube([
            ...
            translate([
                alignment_dm / 2 - alignment_notch_fl + 
                    alignment_notch_dm / 2,
                0,
                -1
            ])
            cylinder(
                d = alignment_notch_dm, 
                h = alignment_h + 2, 
                $fn = 18
            );
        }
    ...
    color("Black") {
        ...
        cylinder( d = second_axle_dm, h = second_axle_h, $fn = 18);
    }
    ...
    module hour_hand() {
        ...
        if (hand_style == "classic") {
            color("Black")
            png_hands( "stunden.png", l, h, [-9.5, -6.75, 0]);
        }
    }
    ...
    module minute_hand() {
        ...
        if (hand_style == "classic") {
            color("Black")
            png_hands("minuten.png", l, h, [-9, -6.75, 0]);
        }
    ...
    module second_hand() {
        ...
        if (hand_style == "classic") {
            color("Black")
            png_hands("sekunden.png", l, h, [-4,-36.8,0] );
        }
    }
